...
const Input: React.FC<InputProps> = ({ label, id, error, className = '', wrapperClassName = '', fieldIdentifier, trailingNode, ...props }) => {
    ...
    <div className={`mb-2 ${wrapperClassName}`}> {/* Changed mb-4 to mb-2 */}
      ...
      <div className="relative">
        <input
          ...
          className={`${baseStyle} ${errorStyle} ${className} ${trailingNode ? 'pr-10' : ''}`}
          {...props}
        />
        {trailingNode && (
          <div className="absolute inset-y-0 right-0 flex items-center pr-2 text-text-secondary dark:text-dark-text-secondary">
            {trailingNode}
          </div>
        )}
      </div>
