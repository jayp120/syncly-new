...
export const MyTasksPage: React.FC = () => {
  ...
  const handleUpdateTask = async (taskId: string, updatedFields: Partial<Task>) => {
    ...
    const updatedTask = { ...updatedTasks[taskIndex], ...updatedFields };
    ...
    try {
      if (updatedFields.status && updatedTask.taskType === TaskType.Team) {
      ...
      // DataService will trigger eventBus, which will cause a refetch for other tabs/components.
      // This local optimistic update makes the current UI feel instant.
    } catch (error) {
      console.error("Failed to update task:", error);
      addToast("Failed to update task. Reverting change.", 'error');
      // Rollback on failure
      setTasks(originalTasks);
      if (taskToView && taskToView.id === taskId) {
        setTaskToView(originalTasks[taskIndex]);
      }
    }
  };
