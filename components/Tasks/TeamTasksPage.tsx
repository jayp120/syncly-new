...
export const TeamTasksPage: React.FC = () => {
  ...
  const handleUpdateTask = async (taskId: string, updatedFields: Partial<Task>) => {
    ...
    updatedTasks[taskIndex] = { ...updatedTasks[taskIndex], ...updatedFields };
    setAllTeamTasks(updatedTasks);
    if (taskToView && taskToView.id === taskId) {
      setTaskToView(updatedTasks[taskIndex]);
    }

    try {
      await DataService.updateTask(taskId, updatedFields, currentUser);
      // DataService triggers eventBus, no need to call fetchData here for this tab.
    } catch (error) {
      console.error("Failed to update task:", error);
      addToast("Failed to update task. Reverting change.", 'error');
      // Rollback on failure
      setAllTeamTasks(originalTasks);
      if (taskToView && taskToView.id === taskId) {
